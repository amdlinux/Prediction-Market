...
export default function AdminMarketList({ markets }: { markets: Market[] }) {
  ...
  async function handleVoid(marketId: string) {
    ...
    try {
      ...
      setResults(prev => ({
        ...prev,
        [marketId]: `✅ Voided — ${data.results.refunded} refunded`
      }))
      router.refresh()
    } catch (err: any) {
      setResults(prev => ({ ...prev, [marketId]: `❌ ${err.message}` }))
        ...
        return (
          <div key={market.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`
                text-xs font-semibold px-2 py-1 rounded-full ml-4
              ...
              `}>
                {market.status}{market.outcome ? ` · ${market.outcome}` : ''}
              </span>
            </div>

            {/* Show live price for open markets */}
            {market.status === 'OPEN' || market.status === 'HALTED' && (
              <div className="flex items-center gap-4 mb-3 text-sm">
                ...
                <span className="text-gray-500 text-xs">
                  ({market.yesShares.toFixed(0)} yes shares,
                  {market.noShares.toFixed(0)} no shares)
                </span>
              </div>
            )}
            ...
            {market.status === 'OPEN' || market.status === 'HALTED' && (
              <div className="flex gap-2">
                ...
                >
                  {loading === `${market.id}-YES` ? 'Settling...' : 'Resolve YES'}
                </button>
                <button
                  ...
                  className="bg-red-800 hover:bg-red-700 disabled:opacity-50
                             text-white text-sm px-4 py-2 rounded-lg transition"
                >
                  {loading === `${market.id}-NO` ? 'Settling...' : 'Resolve NO'}
                </button>
                <button
